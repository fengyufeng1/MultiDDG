...
def train_one_epoch(model, data_loader):
    ...
    for data in data_loader:
        ...
        y_pred = model(node, graph, wt_node, wt_graph, atom, edge, wt_atom, wt_edge)  # y_pred.shape = ()
        ...
        train_pred.append(y_pred.item())  # 将预测值添加到列表中
        train_true.append(y_true.item())  # 将真实值添加到列表中 
...
def train(feature_path, datasets_name, model, train_dataframe, valid_dataframe, fold = 0):
    train_loader = DataLoader(dataset=ProDataset(feature_path, train_dataframe), batch_size=BATCH_SIZE, shuffle=True, num_workers=8)
    valid_loader = DataLoader(dataset=ProDataset(feature_path, valid_dataframe), batch_size=BATCH_SIZE, shuffle=False, num_workers=8)
    ...
    # np.save(File_path + f"CV/cv_loss/{datasets_name}_Fold_{str(fold)}_train_loss.npy", np.array(loss_train_list))
    # np.save(File_path + f"CV/cv_loss/{datasets_name}_Fold_{str(fold)}_val_loss.npy", np.array(loss_val_list))

    return best_epoch, valid_true, valid_pred, best_rmse, best_r

"""Datasets without reverse mutations"""
def cross_validation(feature_path, datasets_name, all_dataframe, fold_number):
    ...
    # np.save(File_path+f'CV/cv_eval_list/{datasets_name}_RMSE.npy', all_RMSE_list)
    # np.save(File_path+f'CV/cv_eval_list/{datasets_name}_R.npy', all_R_list)
    # np.save(File_path+f'CV/cv_eval_list/{datasets_name}_all_y_true.npy', all_y_true)
    # np.save(File_path+f'CV/cv_eval_list/{datasets_name}_all_y_pred.npy', all_y_pred)

    return

"""Datasets containing reverse mutations, precluster"""
def cross_validation_preload(feature_path, datasets_name, all_dataframe):
    ...
    # M1707,S4191,S8338
    f = open(File_path + f'CV/cv_precluster/divided-{datasets_name}.pkl', 'rb')
    divid = pickle.load(f)
    f.close()
    split_folds = []
...
def main(datasets_list, feature_path, use_preloaded_splits):
    print(datasets_list)
    ...
    for datasets in datasets_list:
        ...
        if use_preloaded_splits:
            cross_validation_preload(feature_path, datasets_name, train_dataframe)
        else:
            cross_validation(feature_path, datasets_name, train_dataframe, fold_number = 5)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_preloaded_splits', action='store_true', 
                        help='Use preloaded cross-validation splits.')

    args = parser.parse_args()

    main(args.datasets, args.feature_path, args.use_preloaded_splits)
